 ...
 * Time: 14:22
 */
class InverseOperation[Dimension <: Vector[Dimension]](matrix: MatrixNxN[Dimension]) {
  ...
  private def moveRowWithMaxAbsToTop(matrices: MatrixTuple, column: Int): MatrixTuple = {
    val maxAbs = matrices._1.transpose.apply(column).zipWithIndex.drop(column).maxBy(_._1)
    val maxAbsRowIndex: Int = maxAbs._2

    if (maxAbs._1 == 0) {
      throw new InverseMatrixNotExistsException(matrix)
    }

    if (maxAbsRowIndex == column)
      matrices
    else
      swap(matrices, (column, maxAbsRowIndex))
  }

  private def swap(matrices: MatrixTuple, indices: (Int, Int)): MatrixTuple = {

    def swap(list: List[List[Float]]) =
      List.range(0, list.length) map (row =>
        list(row match {
  ...
  private def multiplyRow(matrices: MatrixTuple, row: Int, multiplier: Float): MatrixTuple = {
    ...
    def multiplyRow(list: List[List[Float]]): List[List[Float]] =
      list.updated(
        row,
        list(row) map (cell => cell * multiplier))
  ...
  private def addRowToRow(matrices: MatrixTuple, addedRow: Int, targetRow: Int, multiplier: Float): MatrixTuple  = {
    ...
    def addRowToRow(list: List[List[Float]]): List[List[Float]] =
      list.updated(
        targetRow,
        list(targetRow) zip list(addedRow) map (tuple => tuple._1 + tuple._2 * multiplier))
  ...
  private def identity(dimension: Int): List[List[Float]] = {
    ...
    List.range(0, dimension) map (row =>
      List.tabulate
        (dimension)
        (cell =>
          if (cell == row)
